...
from ocr.clstm_nouveaupermis import clstm_ocr_nouveaupermis, clstm_ocr_nouveaupermis_parallel, clstm_ocr_calib_nouveaupermis, checkdate, checkdateregex, get_similar
...
def extract_roi(class_name, dets, thresh=0.5):
    ...
    for i in inds:
        ...
        elif class_name=="date_permis":
            print "date"
            bbox[0]+=0.5*hight
            bbox[2]+=0.2*hight

        elif class_name=="date_valide":
            print "date"
            bbox[0]+=0.6*hight
            bbox[2]+=0.25*hight
...
def demo_parallel(net, image_name):
    ...
    for cls_ind, cls in enumerate(CLASSES[2:]):
        ...
        print len(tmp), cls
        if len(tmp) > 0 :# and ( cls=="nom"  or cls=="prenom"): #and cls=="date_valide" or cls=="mrz": # and cls=="date_permis_B":#or cls=='ville' or cls=='marque') :
            ...
            #Process msz
                ...
                txt_mrz, prob_mrz= pytesseract.image_to_string(Image.open(filename)), 1
                if(len(txt_mrz)>17):
                    for i in range(len(txt_mrz)):
                        if not txt_mrz[-1].isdigit(): txt_mrz=txt_mrz[:-1]
                        else:
                            break
                    print txt_mrz
                    txt_mrz= txt_mrz[-11:-9]+"."+ txt_mrz[-13:-11]+"."+"20"+ txt_mrz[-15:-13]
                #res[cls] = (pts_msz, txt_mrz, prob_mrz)
    ...
    for cls in list_cls:
        ...
        if cls=="date_valide" and mrz==True:
            print "date valide error", txt_mrz
            if prob_tmp[cls]<0.8 or checkdateregex(txt_tmp[cls])==False and checkdateregex(txt_mrz):
                txt_tmp[cls]=txt_mrz
                prob_tmp[cls]=1.0
...
def demo2(net, image_name):
    ...
    print "Detection nouveau permis"
    print ('Detection took {:.3f}s for '
           '{:d} object proposals').format(timer.total_time, boxes.shape[0])
...
def detect_nouveaupermis(filename):
    ...
    print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
    print 'Demo for classified nouveau permis...'
    return demo2(net, filename)

def calib_roi(im,bbx,cls):
    #txt, prob = clstm_ocr_carte_grise(im[bbx[1]:bbx[3], bbx[0]:bbx[2]], cls=='lieu')
    txt, prob = clstm_ocr_nouveaupermis(im[bbx[1]:bbx[3], bbx[0]:bbx[2]], cls)
    txt_temp,prob_temp="",0
    cv2.setNumThreads(0)
    h = np.size(im, 0)
    w = np.size(im, 1)

    print "clstm_ocr_nouveaupermis success"
    if(prob<0.99):
        for i in range(1,4):    
            for j in range(1,3):
                ...
                #         txt_temp,prob_temp=clstm_ocr_calib_permis(im[bbx[1]-5*i*math.pow( -1, j):bbx[3]-5*i*math.pow( -1, j), bbx[0]-3*i*math.pow( -1, j):bbx[2]+3*i*math.pow( -1, j)], cls)
                if (bbx[1]>15) and ( bbx[3] >15) and (bbx[2]>9) and (bbx[0]>9):
                    txt_temp,prob_temp=clstm_ocr_calib_nouveaupermis(im[bbx[1]-5*i*math.pow( -1, j):bbx[3]-5*i*math.pow( -1, j), bbx[0]-3*i*math.pow( -1, j):bbx[2]-3*i*math.pow( -1, j)], cls)
                    if(prob<prob_temp) and len(txt_temp)>=2 and checkdateregex(txt_temp,cls):
	                    txt=txt_temp
	                    prob=prob_temp
                print "cls=",cls
                print "clstm_ocr_calib_nouveaupermis success"
