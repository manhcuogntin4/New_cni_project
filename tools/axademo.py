...
from ocr.clstm import clstm_ocr, clstm_ocr_parallel
...
def ocr_queue(im, bbx, cls, q):
    print "ocr queue start"
    q.put(calib_roi(im,bbx,cls))
    print "ocr queue finish"
...
def check(boxes, scores, thresh=0.8, nms_thresh=0.3):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        dets = dets[keep, :]
        inds = np.where(dets[:, -1] >= thresh)[0]
        if cls_ind ==7:  #we skip 'nom epouse' check
            continue
        if len(inds) == 0:
            return False
    return True
...
def detect_cni(filename):
    ...
    prototxt = os.path.join(cfg.MODELS_DIR, NETS[args.demo_net][0],
                            'faster_rcnn_alt_opt', 'test.prototxt')
        ...
        for i in range(0,2):
            for j in range(0,2):
                if (bbx[1]>15) and ( bbx[3] >15) and (bbx[2]>9) and (bbx[0]>9):
                    txt_temp,prob_temp=clstm_ocr_calib(im[bbx[1]-5*i*math.pow( -1, j):bbx[3]+5*i*math.pow( -1, j), bbx[0]-3*i*math.pow( -1, j):bbx[2]+3*i*math.pow( -1, j)], cls)
                    if(prob<prob_temp):
                        txt=txt_temp
                        prob=prob_temp
    #TODO: Process image 
